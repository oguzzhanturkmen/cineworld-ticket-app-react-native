...
export default function SeatScreen() {
        ...
        if (seat.id === seatId) {
          if (seat.status !== "reserved") {
            if (
              ...
              newSelectedSeatIds.length < totalSeats
            ) {
              seat.status = "selected";
              newSelectedSeatIds.push(seatId);
            } else if (seat.status === "selected") {
              seat.status = "available";
              newSelectedSeatIds = newSelectedSeatIds.filter(
                (id) => id !== seatId
              );
            }
          }
  ...
  const getSeats = async (screenId, showtimeId) => {
    try {
      ...
      const reservationData = await getSeatReservationInfoByShowtimeId(
        showtimeId
      );
      if (seatData && reservationData) {
        ...
        console.error(
          "API returned null or undefined for seats or reservations"
        );
      }
    } catch (error) {
  ...
  return (
    <View className="flex-1 bg-neutral-900 h-full">
      ...
      <TouchableOpacity
        className="flex-row justify-center items-end   mx-4 mb-6 p-3"
