    ...
    <View className="flex-1 bg-neutral-900 h-full ">
      <SafeAreaView className="mb-2 z-10">
        ...
        <View
          ...
          onPress={() => navigation.navigate("ChooseTheater", { item })}
        >
          <View className="flex-row items-center  mx-5 justify-between ">
            <View>
              <TextInput
        ...
        <Text className="text-white text-sm font-bold mx-4 mt-4 ">
          All Theaters
        </Text>
        <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
          {theaters.map((theater) => (
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate("ChooseDateAndShowtime", {
                  item,
                  theater,
                  movie,
                })
      ...
      <Modal isVisible={isModalVisible}>
        <View style={{ flex: 1, marginTop: height * 0.35 }}>
          <Text className="text-white text-sm font-bold mx-4 mt-4 text-center">
            All Cities
          </Text>
          {allTheaters.map((theater) => (
            <TouchableOpacity
              className="flex-row justify-center items-center mb-4 mt-2"
              style={{
                height: height * 0.07,
                backgroundColor: "#393939",
                borderRadius: 20,
              }}
              onPress={handleCitySelection(theater.location)}
            >
              <View className="flex-row justify-center items-center">
                <Text className="text-white text-xl font-bold mx-2 mx-4">
                  {theater.location}
                </Text>
              </View>
            </TouchableOpacity>
